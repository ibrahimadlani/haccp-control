...
async def main() -> None:
    ...
    async with AsyncSessionLocal() as s:
        ...
        site = await upsert(
            ...
            settings={
                "timeclock": {"enabled": True, "applies_to_managers": False}
            },
        )

        # ── Rôles ─────────────────────────────────────────────────────────────
        role_manager = await upsert(
            s, Role, ROLE_MANAGER_ID,
            nom_role="MANAGER",
        ...
        role_chef = await upsert(
            ...
            permissions={"can_create_temperature_record": True, "can_clock": True, "can_receive": True},
        )
        role_op = await upsert(
            s, Role, ROLE_OPERATEUR_ID,
        ...
        manager = await upsert(
            s, Utilisateur, USER_MANAGER_ID,
            nom="Leblanc", prenom="Marie",
            email=MANAGER_EMAIL,
        ...
        chef = await upsert(
            s, Utilisateur, USER_CHEF_ID,
            nom="Rousseau", prenom="Jean-Pierre",
            email=CHEF_EMAIL,
        ...
        operator = await upsert(
            s, Utilisateur, USER_OPERATOR_ID,
            nom="Diallo", prenom="Aïcha",
        ...
        await upsert_assignment(s, manager.id, site.id, role_manager.id, "Gestionnaire de restauration")
        ...
        # ── Équipements ───────────────────────────────────────────────────────
        EQUIP = [
            (UUID("77777777-7777-4777-8777-777777777771"), "Chambre froide positive — Viandes",
             "CHAMBRE_FROIDE_POSITIVE", Decimal("0"), Decimal("4")),
            (UUID("77777777-7777-4777-8777-777777777772"), "Chambre froide positive — Légumes & Fruits",
             "CHAMBRE_FROIDE_POSITIVE", Decimal("4"), Decimal("8")),
            (UUID("77777777-7777-4777-8777-777777777773"), "Chambre froide négative — Surgelés",
             "CHAMBRE_FROIDE_NEGATIVE", Decimal("-25"), Decimal("-18")),
            (UUID("77777777-7777-4777-8777-777777777774"), "Réfrigérateur — Produits laitiers",
             "REFRIGERATEUR_VIANDE", Decimal("2"), Decimal("6")),
            (UUID("77777777-7777-4777-8777-777777777775"), "Vitrine réfrigérée — Self",
             "VITRINE_REFRIGEREE", Decimal("2"), Decimal("6")),
            (UUID("77777777-7777-4777-8777-777777777776"), "Cellule de refroidissement rapide",
             "CELLULE_REFROIDISSEMENT", Decimal("0"), Decimal("3")),
            (UUID("77777777-7777-4777-8777-777777777777"), "Réfrigérateur — Poissons",
             "REFRIGERATEUR_POISSON", Decimal("0"), Decimal("2")),
        ]
        equip_objects = {}
        ...
        SUPPLIERS_DATA = [
            (UUID("aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa"),
             "Boucherie Centrale Île-de-France", "France", "75012300000012", "approved",
             "94 avenue du Général Leclerc, 94000 Créteil", "Créteil",
             "Hugues Delorme", "h.delorme@boucherie-idf.fr", "+33148980011",
             "IFS Food", "Agrément sanitaire FR 94.001.001 CE"),
            (UUID("bbbbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbbbb"),
             "Maraîchers Bio de Beauce", "France", "28001560000037", "approved",
             "Route des Champs, 28300 Saint-Prest", "Saint-Prest",
             "Sylvie Chartier", "contact@bio-beauce.fr", "+33237412289",
             "AB Agriculture Biologique", "Certification Ecocert n°28-123"),
            (UUID("cccccccc-cccc-4ccc-8ccc-cccccccccccc"),
             "Coopérative Laitière de Normandie", "France", "14001420000029", "approved",
             "Chemin des Herbages, 14120 Mondeville", "Mondeville",
             "Patrick Lebreton", "p.lebreton@coop-normandie.fr", "+33231800055",
             "IFS Food", "Quota hebdomadaire confirmé chaque lundi"),
            (UUID("dddddddd-dddd-4ddd-8ddd-dddddddddddd"),
             "Marée de Rungis", "France", "94046000000081", "approved",
             "Pavillon de la Marée, MIN de Rungis, 94150 Rungis", "Rungis",
             "Ahmed Tazi", "a.tazi@maree-rungis.fr", "+33149785050",
             "MSC Pêche Durable", "Livraisons lundi + jeudi matin avant 6h"),
            (UUID("eeeeeeee-eeee-4eee-8eee-eeeeeeeeeeee"),
             "Boulangerie Artisanale Moreau", "France", "77001080000016", "approved",
             "3 place du Marché, 77100 Meaux", "Meaux",
             "Éric Moreau", "eric@boulangerie-moreau.fr", "+33164340987",
             None, "Pain livré chaque matin avant 7h30"),
            (UUID("ffffffff-ffff-4fff-8fff-ffffffffffff"),
             "Metro Cash & Carry Roissy", "France", "95001560000047", "approved",
             "Zone Commerciale Paris Nord 2, 95912 Roissy-en-France", "Roissy-en-France",
             "Service commercial", "b2b@metro.fr", "+33800880099",
             None, "Commandes en ligne — livraison J+1"),
        ]
        supplier_objects = {}
        for sid, name, country, siret, status, addr, city, cname, cemail, cphone, cert, notes in SUPPLIERS_DATA:
            sup = await upsert(
                s, Supplier, sid,
        ...
        PRODUCTS_DATA = [
            ...
            (UUID("11111111-aaaa-4aaa-8aaa-aaaaaaaaaaaa"), SUPP_VIANDES,
             "Escalope de dinde", "VIA-001", True, 0.0, 4.0),
            (UUID("22222222-aaaa-4aaa-8aaa-aaaaaaaaaaaa"), SUPP_VIANDES,
             "Steak haché surgelé 15%MG", "VIA-002", True, -25.0, -18.0),
            (UUID("33333333-aaaa-4aaa-8aaa-aaaaaaaaaaaa"), SUPP_VIANDES,
             "Poulet fermier entier", "VIA-003", True, 0.0, 4.0),
            (UUID("44444444-aaaa-4aaa-8aaa-aaaaaaaaaaaa"), SUPP_VIANDES,
             "Jambon cuit supérieur tranché", "VIA-004", True, 2.0, 6.0),

            (UUID("11111111-bbbb-4bbb-8bbb-bbbbbbbbbbbb"), SUPP_BIO,
             "Carottes bio (sac 5kg)", "BIO-001", False, None, None),
            (UUID("22222222-bbbb-4bbb-8bbb-bbbbbbbbbbbb"), SUPP_BIO,
             "Pommes de terre bio", "BIO-002", False, None, None),
            (UUID("33333333-bbbb-4bbb-8bbb-bbbbbbbbbbbb"), SUPP_BIO,
             "Salade frisée bio", "BIO-003", True, 4.0, 8.0),
            (UUID("44444444-bbbb-4bbb-8bbb-bbbbbbbbbbbb"), SUPP_BIO,
             "Tomates cerises bio", "BIO-004", True, 8.0, 12.0),

            (UUID("11111111-cccc-4ccc-8ccc-cccccccccccc"), SUPP_LAITIER,
             "Lait demi-écrémé UHT (colis 6×1L)", "LAI-001", False, None, None),
            (UUID("22222222-cccc-4ccc-8ccc-cccccccccccc"), SUPP_LAITIER,
             "Yaourt nature (carton 24)", "LAI-002", True, 2.0, 6.0),
            (UUID("33333333-cccc-4ccc-8ccc-cccccccccccc"), SUPP_LAITIER,
             "Emmental râpé (500g)", "LAI-003", True, 2.0, 8.0),
            (UUID("44444444-cccc-4ccc-8ccc-cccccccccccc"), SUPP_LAITIER,
             "Beurre doux plaquette (250g)", "LAI-004", True, 2.0, 6.0),

            (UUID("11111111-dddd-4ddd-8ddd-dddddddddddd"), SUPP_MAREE,
             "Filet de cabillaud frais", "MAR-001", True, 0.0, 2.0),
            (UUID("22222222-dddd-4ddd-8ddd-dddddddddddd"), SUPP_MAREE,
             "Crevettes entières surgelées", "MAR-002", True, -25.0, -18.0),
            (UUID("33333333-dddd-4ddd-8ddd-dddddddddddd"), SUPP_MAREE,
             "Saumon portions surgelées", "MAR-003", True, -25.0, -18.0),

            (UUID("11111111-eeee-4eee-8eee-eeeeeeeeeeee"), SUPP_BOULANG,
             "Baguette tradition (lot 10)", "BOU-001", False, None, None),
            (UUID("22222222-eeee-4eee-8eee-eeeeeeeeeeee"), SUPP_BOULANG,
             "Pain de mie tranché (500g)", "BOU-002", False, None, None),

            (UUID("11111111-ffff-4fff-8fff-ffffffffffff"), SUPP_METRO,
             "Huile de tournesol (bidon 5L)", "MET-001", False, None, None),
            (UUID("22222222-ffff-4fff-8fff-ffffffffffff"), SUPP_METRO,
             "Riz long grain (sac 5kg)", "MET-002", False, None, None),
            (UUID("33333333-ffff-4fff-8fff-ffffffffffff"), SUPP_METRO,
             "Farine T45 (sac 5kg)", "MET-003", False, None, None),
        ]
        product_objects = {}
        ...
        ROUTINES_DATA = [
            (UUID("b0000001-0000-4000-8000-000000000001"), "Ouverture — Mise en place",        ScheduleType.OPENING),
            (UUID("b0000001-0000-4000-8000-000000000002"), "Fermeture Midi — Nettoyage service", ScheduleType.CLOSING),
            (UUID("b0000001-0000-4000-8000-000000000003"), "Grand Nettoyage Hebdomadaire",      ScheduleType.WEEKLY),
        ]
        routine_objects = {}
        for rid, rname, rtype in ROUTINES_DATA:
            routine_objects[rid] = await upsert(
                s, CleaningRoutine, rid,
                establishment_id=site.id,
                name=rname,
                schedule_type=rtype,
            )
        ...
        TASKS_DATA = [
            # Ouverture
            (UUID("c0000001-0000-4000-8000-000000000001"), R_OUVERTURE, Z_CHAUD,
             "Vérifier la propreté des plans de travail", "S'assurer de l'absence de résidus de la veille."),
            (UUID("c0000001-0000-4000-8000-000000000002"), R_OUVERTURE, Z_CHAUD,
             "Allumer et vérifier les fours", "Contrôle visuel + test de chauffe."),
            (UUID("c0000001-0000-4000-8000-000000000003"), R_OUVERTURE, Z_FROID,
             "Contrôler les températures des enceintes froides", "Noter les relevés sur le registre HACCP."),
            (UUID("c0000001-0000-4000-8000-000000000004"), R_OUVERTURE, Z_CF,
             "Vérifier les joints des portes de CF", "Signaler tout joint abîmé au gestionnaire."),
            (UUID("c0000001-0000-4000-8000-000000000005"), R_OUVERTURE, Z_SALLE,
             "Disposer les sets de table et couverts", None),
            (UUID("c0000001-0000-4000-8000-000000000006"), R_OUVERTURE, Z_PLONGE,
             "Mettre en route le lave-vaisselle industriel", "Vérifier le niveau de sel et rinçage."),

            # Fermeture Midi
            (UUID("c0000001-0000-4000-8000-000000000010"), R_FERMETURE, Z_CHAUD,
             "Nettoyer et désinfecter les fours", "Produit agréé P3 — contact 15 min minimum."),
            (UUID("c0000001-0000-4000-8000-000000000011"), R_FERMETURE, Z_CHAUD,
             "Nettoyer la friteuse et vider l'huile", "Filtrer l'huile si < 20 utilisations."),
            (UUID("c0000001-0000-4000-8000-000000000012"), R_FERMETURE, Z_CHAUD,
             "Désinfecter les plans de travail inox", "Spray désinfectant + lavette à usage unique."),
            (UUID("c0000001-0000-4000-8000-000000000013"), R_FERMETURE, Z_FROID,
             "Ranger et filmer tous les aliments entamés", "Étiqueter avec date et heure."),
            (UUID("c0000001-0000-4000-8000-000000000014"), R_FERMETURE, Z_FROID,
             "Nettoyer et désinfecter le plan de découpe", "Changer les planches si abîmées."),
            (UUID("c0000001-0000-4000-8000-000000000015"), R_FERMETURE, Z_PLONGE,
             "Désinfecter le bac de plonge", "Produit chloré dilué 0,5%."),
            (UUID("c0000001-0000-4000-8000-000000000016"), R_FERMETURE, Z_PLONGE,
             "Passer la raclette et le balai sur le sol plonge", None),
            (UUID("c0000001-0000-4000-8000-000000000017"), R_FERMETURE, Z_SALLE,
             "Débarrasser et essuyer les tables", "Produit désinfectant multi-surfaces."),
            (UUID("c0000001-0000-4000-8000-000000000018"), R_FERMETURE, Z_SALLE,
             "Balayer et passer la serpillère salle", None),
            (UUID("c0000001-0000-4000-8000-000000000019"), R_FERMETURE, Z_QUAI,
             "Sortir les poubelles et rincer les bacs", "Tri sélectif — vérifier les sacs."),

            # Hebdo
            (UUID("c0000001-0000-4000-8000-000000000020"), R_HEBDO, Z_CHAUD,
             "Démonter et nettoyer les grilles de ventilation", "Trempage eau chaude + brosse."),
            (UUID("c0000001-0000-4000-8000-000000000021"), R_HEBDO, Z_CHAUD,
             "Dégraisser les hottes aspirantes", "Produit dégraissant puissant — EPI obligatoires."),
            (UUID("c0000001-0000-4000-8000-000000000022"), R_HEBDO, Z_CF,
             "Nettoyer l'intérieur des chambres froides", "Eau chaude + désinfectant alimentaire."),
            (UUID("c0000001-0000-4000-8000-000000000023"), R_HEBDO, Z_CF,
             "Défrosting du congélateur si nécessaire", "Débrancher 30 min, éponger."),
            (UUID("c0000001-0000-4000-8000-000000000024"), R_HEBDO, Z_PLONGE,
             "Détartrer le lave-vaisselle", "Produit détartrant — cycle à vide."),
            (UUID("c0000001-0000-4000-8000-000000000025"), R_HEBDO, Z_VEST,
             "Nettoyer et désinfecter les sanitaires personnel", "Détartrant + désinfectant WC."),
            (UUID("c0000001-0000-4000-8000-000000000026"), R_HEBDO, Z_QUAI,
             "Laver le quai de livraison au karcher", None),
        ]
        task_objects = {}
        for tid, rid, zid, tname, tdesc in TASKS_DATA:
            task_objects[tid] = await upsert(
                s, CleaningTaskTemplate, tid,
                routine_id=rid,
                zone_id=zid,
                name=tname,
                description=tdesc,
        ...
        # ── Pointages (5 jours) ───────────────────────────────────────────────
        timeclock_events = []
        for dago, user_id in [(0, chef.id), (0, agent1.id), (1, chef.id), (1, agent1.id), (2, chef.id)]:
            timeclock_events += [
                Pointage(etablissement_id=site.id, utilisateur_id=user_id,
        ...
        if existing.scalar_one_or_none() is None:
            ...
            await s.flush()
            items1 = [
                ReceptionItem(session_id=sess1.id, product_id=UUID("11111111-aaaa-4aaa-8aaa-aaaaaaaaaaaa"),
                              lot_number="VIA24-0610", dluo=date.today() + timedelta(days=3),
                              measured_temperature=2.8, is_compliant=True,
                              scanned_at=days_ago(1, 7, 50)),
                ReceptionItem(session_id=sess1.id, product_id=UUID("33333333-aaaa-4aaa-8aaa-aaaaaaaaaaaa"),
                              lot_number="VIA24-0611", dluo=date.today() + timedelta(days=2),
                              measured_temperature=3.5, is_compliant=True,
                              scanned_at=days_ago(1, 7, 52)),
                ReceptionItem(session_id=sess1.id, product_id=UUID("22222222-cccc-4ccc-8ccc-cccccccccccc"),
                              lot_number="LAI24-0890", dluo=date.today() + timedelta(days=14),
                              measured_temperature=4.1, is_compliant=True,
        ...
        if existing2.scalar_one_or_none() is None:
            ...
            await s.flush()
            items2 = [
                ReceptionItem(session_id=sess2.id, product_id=UUID("11111111-dddd-4ddd-8ddd-dddddddddddd"),
                              lot_number="MAR24-1102", dluo=date.today() + timedelta(days=2),
                              measured_temperature=4.2, is_compliant=False,
                              nc_id=nc_reception.id,
                              scanned_at=days_ago(2, 6, 40)),
                ReceptionItem(session_id=sess2.id, product_id=UUID("22222222-dddd-4ddd-8ddd-dddddddddddd"),
                              lot_number="MAR24-1103", dluo=date.today() + timedelta(days=60),
                              measured_temperature=-20.0, is_compliant=True,
